# coding=utf-8
"""
Compute TextEmb for classification/ regression/ question-answering tasks (Vu et al., 2020).
...
from transformers import AutoConfig, AutoModelWithHeads, AutoTokenizer, PreTrainedModel
...
def compute_textemb(dataset_manager: DatasetManager, model: PreTrainedModel, run_args: RunArguments):
    """ Compute the TextEmb embedding """
...
def run_textemb(args, data_args=None, run_args=None, seed=42):
    ...
    # Load
    model = AutoModelWithHeads.from_pretrained(args["model_name"], config=model_config)
    dataset_manager.load_and_preprocess()
    model.to(run_args.device)

    feature_dict = compute_textemb(dataset_manager, model, run_args)
...
def main():
    ...
    parser.add_argument("--output_dir", default="output/textemb", type=str)
    parser.add_argument("--overwrite", action="store_true", default=False)

    args = parser.parse_args()

    run_textemb(vars(args))
